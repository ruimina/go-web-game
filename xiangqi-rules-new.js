...
function inBounds(x, y) {
  return x >= 0 && x >= 0 && x < XQ_BOARD.WIDTH && y < XQ_BOARD.HEIGHT;
}
...
// 升级炮：计算目标格距离起点的格数
function getCannonTargetDistance(fromX, fromY, toX, toY) {
  if (fromX === toX) {
  ...
  if (piece.upgraded) {
    switch (piece.type) {
      ...
      case XQ_PIECE.CANNON: {
        // 升级炮：只能打原本位置和其后一格（最多移动2格）
        if (fromX !== toX && fromY !== toY) return false;
        const distance = getCannonTargetDistance(fromX, fromY, toX, toY);
...
function simulateMove(state, fromX, fromY, toX, toY) {
  ...
  // 吃掉升级棋子也获得升级
  if (target && target.upgraded && target.team !== XQ_TEAM.NEUTRAL) {
    piece.upgraded = true;
  }
...
export function moveXiangqi(state, fromX, fromY, toX, toY) {
  ...
  next.lastMove = {
    ...
    capturedUpgraded: captured?.upgraded ?? false
  };
